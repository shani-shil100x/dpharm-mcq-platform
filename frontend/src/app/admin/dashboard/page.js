...
export default function AdminDashboard() {
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-6xl">
      <h1 className="text-3xl font-extrabold text-white mb-6 transition-colors">Admin Dashboard</h1>
        ...
        <div className="flex overflow-x-auto border-b border-slate-700 transition-colors">
          {tabs.map(({ key, label, icon: Icon }) => (
            <button
              ...
              className={`flex-1 min-w-[140px] py-4 text-sm font-medium flex items-center justify-center gap-2 whitespace-nowrap transition-colors ${activeTab === key ? 'border-b-2 border-emerald-500 text-emerald-400 bg-emerald-900/20' : 'text-gray-400 hover:text-gray-200 hover:bg-slate-700/50'}`}
            >
          ...
          {activeTab === 'subjects' && (
            <>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                  <h2 className="text-xl font-bold text-white transition-colors">Subjects</h2>
                  <p className="text-sm text-gray-400 mt-1 transition-colors">Add, rename, change icon, or delete D.Pharm subjects.</p>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-slate-700 transition-colors">
                    ...
                    <tbody className="divide-y divide-slate-700 transition-colors">
                      ...
                      {subjects.length === 0 && <tr><td colSpan={4} className="text-center text-gray-500 py-10">No subjects found. Add one above.</td></tr>}
                    </tbody>
                  </table>
                </div>
              )}
          ...
          {activeTab === 'upload' && (
            <>
              <div className="max-w-4xl">
                <div className="mb-6">
                  <h2 className="text-xl font-bold text-white transition-colors">Bulk Upload MCQs</h2>
                  <p className="text-sm text-gray-400 mt-1 transition-colors">Paste 30-40 questions at once using the specific format below.</p>
                </div>

                <div className="bg-emerald-900/20 border border-emerald-800/50 rounded-lg p-4 mb-8 transition-colors">
                  <h3 className="text-sm font-semibold text-emerald-300 mb-2">Required Format Example:</h3>
                  <pre className="text-xs text-emerald-400 font-mono bg-slate-800/50 p-3 rounded border border-emerald-800/50 overflow-x-auto whitespace-pre-wrap transition-colors">
...
Answer: B. Carbon Dioxide`}
                ...
                {uploadStatus.message && (
                  <div className={`mb-6 p-4 rounded-lg flex items-start text-sm transition-colors ${uploadStatus.type === 'success' ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'}`}>
                ...
                <form onSubmit={handleBulkUpload} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2 transition-colors">Paste Questions</label>
                    <textarea
                      value={rawText}
                      onChange={(e) => setRawText(e.target.value)}
                      rows={14}
                      required
                      className="block w-full border border-slate-600 bg-slate-700 text-white rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-emerald-500 font-mono text-sm transition-colors"
            ...
            <>
              ...
              {loading ? <Loader2 className="h-8 w-8 text-emerald-600 animate-spin mx-auto my-10" /> : selectedSubjectId ? (
                <div className="space-y-4">
                  ...
                  {mcqs.map((q, idx) => (
                    <div key={q._id} className="border border-slate-700 p-4 rounded-xl flex flex-col md:flex-row justify-between gap-4 group hover:border-emerald-600/50 transition-colors">
                      <div className="flex-1">
                        <p className="font-semibold text-gray-200 text-sm mb-2 transition-colors"><span className="text-emerald-400 mr-1">Q{idx + 1}.</span>{q.questionText}</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-gray-300">
                          {q.options.map((opt, i) => (
                            <div key={i} className={`p-2 rounded transition-colors ${opt === q.correctAnswer ? 'bg-emerald-900/40 font-medium text-emerald-300 border border-emerald-700/50' : 'bg-slate-700/50 border border-slate-600'}`}>
                              {opt}
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="flex flex-row md:flex-col gap-2 justify-start items-center">
                        <button onClick={() => handleOpenMcqModal(q)} className="p-2 text-blue-400 hover:bg-blue-900/30 rounded-lg transition-colors"><Edit2 className="h-4 w-4" /></button>
                        <button onClick={() => handleDeleteMcq(q._id)} className="p-2 text-red-400 hover:bg-red-900/30 rounded-lg transition-colors"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'users' && (
            <>
              <div className="mb-6">
                <h2 className="text-xl font-bold text-white transition-colors">Registered Users</h2>
                <p className="text-sm text-gray-400 mt-1 transition-colors">Total platform users: <span className="font-bold text-emerald-400">{users.length}</span></p>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-700 transition-colors">
                      {users.map((u) => (
                        <tr key={u._id} className="hover:bg-slate-700/50 transition-colors">
                          <td className="px-6 py-4 text-sm font-medium text-gray-200">{u.name}</td>
                          <td className="px-6 py-4 text-sm text-gray-400">{u.email}</td>
                          <td className="px-6 py-4 text-sm">
                            <span className={`px-2 py-1 text-xs rounded-full font-medium ${u.role === 'admin' ? 'bg-purple-900/40 text-purple-300' : 'bg-slate-600 text-gray-300'}`}>
                              {u.role}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-4 text-sm text-right">
                            {u.role === 'admin' ? (
                              ...
                              <button
                                ...
                                className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-red-400 bg-red-900/20 rounded-lg hover:bg-red-900/40 transition-colors"
                              >
                      ...
                      {users.length === 0 && <tr><td colSpan={5} className="text-center text-gray-500 py-10">No users found.</td></tr>}
                    </tbody>
                  </table>
                </div>
              )}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm transition-colors">
            <div className="bg-slate-800 rounded-2xl p-6 max-w-md w-full shadow-2xl transition-colors">
              <h3 className="text-xl font-bold text-white mb-4">{subjectId ? 'Edit Subject' : 'Add Subject'}</h3>
        ...
        {isMcqModalOpen && (
          <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm transition-colors">
            <div className="bg-slate-800 rounded-2xl p-6 max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto transition-colors">
              ...
              <form onSubmit={handleSaveMcq} className="space-y-4">
                <div>
                  ...
                  <textarea required value={questionText} onChange={(e) => setQuestionText(e.target.value)} className="block w-full border border-slate-600 bg-slate-700 text-white rounded-lg py-2 px-3 focus:ring-2 focus:ring-emerald-500 min-h-[80px] transition-colors" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {options.map((opt, idx) => (
                    <div key={idx}>
                      <label className="block text-xs font-medium text-gray-400 mb-1">Option {String.fromCharCode(65 + idx)}</label>
                      <input type="text" required value={opt} onChange={(e) => handleOptionChange(idx, e.target.value)} className="block w-full border border-slate-600 bg-slate-700 text-white rounded-lg py-2 px-3 focus:ring-2 focus:ring-emerald-500 text-sm transition-colors" />
                    </div>
                  ))}
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Correct Answer (exact match)</label>
                  <input type="text" required value={correctAnswer} onChange={(e) => setCorrectAnswer(e.target.value)} className="block w-full border border-emerald-700/50 bg-emerald-900/20 text-emerald-300 rounded-lg py-2 px-3 focus:ring-2 focus:ring-emerald-500 font-medium transition-colors" />
                  <p className="text-xs text-emerald-400 mt-1">This must perfectly match one of the 4 options above.</p>
                </div>

                <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-700">
                  <button type="button" onClick={() => setIsMcqModalOpen(false)} className="px-5 py-2 text-sm font-medium text-gray-300 bg-slate-700 border border-slate-600 rounded-lg hover:bg-slate-600 transition-colors">Cancel</button>
